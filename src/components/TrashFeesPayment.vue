        ...
        <v-container>
            ...
            <template>
                <v-row>
                    <v-col>
                        <v-card flat >
                            <v-card-title class="heading-1 blue-grey lighten-4  blue-grey--text text--darken-2">Trashed Fees Payments
                                <v-spacer></v-spacer>
                                ...
                                <div>
                                    <v-data-table :headers="headers" :items="paymentDetails" :search="search" :sort-by="['feeID']" :sort-desc="true">
                                        <template v-slot:[`item.actions`]="{ item }">
                                            <v-card-actions>
                                                <app-RestorePaymentDetailsTrashed @success="updateSuccessAlert($event)" @failed="updateFaileAlert($event)" :payment='item'></app-RestorePaymentDetailsTrashed>
                                                <v-spacer></v-spacer>
                                                <app-DeletePaymentDetailsTrashed @success="deleteAlert($event)" @failed="faileAlert($event)" class="ml-5" :payment='item'></app-DeletePaymentDetailsTrashed>
                                            </v-card-actions>        
                                        </template>
                                    </v-data-table>
                                </div>
...
<script>
    ...
    import DeletePaymentDetailsTrashed from './DeletePaymentDetailsTrashed.vue'
    import RestorePaymentDetailsTrashed from './RestorePaymentDetailsTrashed.vue'

    export default {
        components:{
            'app-DeletePaymentDetailsTrashed':DeletePaymentDetailsTrashed,
            'app-RestorePaymentDetailsTrashed':RestorePaymentDetailsTrashed
        },
        data () {
            return {
        ...
        methods: {
            ...
            getPaymentDetails(){
                this.axios.get(this.$apiUrl+"/api/v1.0/FeeManagement/fees/trashed",{
                params:{
                    date: this.month
                }
