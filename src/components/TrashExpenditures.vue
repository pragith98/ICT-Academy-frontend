        ...
        <v-container>
            ...
            <template>
                <v-row>
                    <v-col>
                        <v-card flat >
                            <v-card-title class="heading-1 blue-grey lighten-4  blue-grey--text text--darken-2">Trashed Expenditures
                                <v-spacer></v-spacer>
                                ...
                                <div>
                                    <v-data-table :headers="headers" :items="paymentDetails" :search="search" :sort-by="['feeID']" :sort-desc="true">
                                        <template v-slot:[`item.actions`]="{ item }">
                                            <v-card-actions>
                                                <app-RestoreExpenditureTrashed @success="updateSuccessAlert($event)" @failed="updateFaileAlert($event)" :payment='item'></app-RestoreExpenditureTrashed>
                                                <v-spacer></v-spacer>
                                                <app-DeleteExpenditureTrashed @success="deleteAlert($event)" @failed="faileAlert($event)" class="ml-5" :payment='item'></app-DeleteExpenditureTrashed>
                                            </v-card-actions>        
                                        </template>
                                    </v-data-table>
                                </div>
...
<script>
    ...
    import DeleteExpenditureTrashed from './DeleteExpenditureTrashed.vue'
    import RestoreExpenditureTrashed from './RestoreExpenditureTrashed.vue'

    export default {
        components:{
            'app-DeleteExpenditureTrashed':DeleteExpenditureTrashed,
            'app-RestoreExpenditureTrashed':RestoreExpenditureTrashed
        },
        data () {
            return {
                ...
                headers: [
                    ...
                    { text: 'AMOUNT(Rs.)',sortable: true, value: 'expenseAmount' },
        ...
        methods: {
            ...
            getPaymentDetails(){
                this.axios.get(this.$apiUrl+"/api/v1.0/ExpenditureManagement/expenditures/trashed",{
                params:{
                    date: this.month
                }
                
                }).then(Response=>(
                    this.paymentDetails=Response.data.expense.data
                ))
            },
