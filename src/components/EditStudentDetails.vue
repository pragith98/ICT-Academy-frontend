  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" scrollable max-width="700px" persistent>
        <template v-slot:activator="{ on, attrs }">
            <v-btn class="primary" fab dark small depressed  v-bind="attrs" v-on="on"><v-icon dark>mdi-account-edit</v-icon></v-btn>
        </template>
        <v-card max-width="700" flat>
        <v-card-title class="heading-1 blue lighten-4 primary--text">Edit Student Details</v-card-title>
        <v-divider></v-divider>
        ...
        <v-card-actions>
            ...
            <v-btn :disabled="!valid" color="primary" @click="Register(),scrollToTop()" depressed>Save</v-btn>
